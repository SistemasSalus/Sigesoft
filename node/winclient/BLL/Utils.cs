    ...
    public static class Utils
    {
        ...
        public static int GetNextSecuentialId(int pintNodeId, int pintTableId)
        {
            ...
            else
            {
                ...
                objSecuential.i_SecuentialId = 1;
        ...
        public static int GetNextGroupOfSecuentialId(int pintNodeId, int pintTableId, int pintCantidadSecuenciales)
        {
            SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
            ...
            var objsecuentialreturn = objSecuential;

            // Actualizar el campo con el nuevo valor a efectos de reservar el ID autogenerado para evitar colisiones entre otros nodos
            if (objSecuential != null)
            {
                objsecuentialreturn.i_SecuentialId = objSecuential.i_SecuentialId + 1;
                //objSecuential.i_SecuentialId = objSecuential.i_SecuentialId + pintCantidadSecuenciales;
            }
            else
            {
                objSecuential = new secuential();
            ...
            return objsecuentialreturn.i_SecuentialId.Value;
        }

        public static int GetNextGroupOfSecuentialIdwithSave(int pintNodeId, int pintTableId, int pintCantidadSecuenciales)
        {
            SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
        ...
        //public static string DevolverValueParametro(int pintGroupId, int pintParameterId)
        //{
        //    //mon.IsActive = true;
        ...
        //        var query = (from a in dbContext.systemparameter
        //                    where a.i_GroupId == pintGroupId && a.i_IsDeleted == 0 && a.i_ParameterId == pintParameterId
        //                    select new
        //                     {
        //                         value = a.v_Value1
        //                     }).FirstOrDefault();
        ...
        public static List<KeyValueDTO> GetProfessional(ref OperationResult pobjOperationResult, string pstrSortExpression)
        {
            ...
            try
            {
                ...
                var query = (from a in dbContext.systemuser
                            join b in dbContext.systemuserrolenode on a.i_SystemUserId equals b.i_SystemUserId
                            where a.i_IsDeleted == 0 && a.v_PersonId.StartsWith("N001-") && b.i_RoleId != 7
        ...
        public static List<KeyValueDTO> GetAllEmpresaPadre(ref OperationResult pobjOperationResult)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                ...
                var query = (from a in dbContext.organization
                             join b in dbContext.organization on a.v_OrganizationPadreId equals b.v_OrganizationId
                             where a.i_IsDeleted == 0 && a.v_OrganizationPadreId != null
                             select new KeyValueDTO
                             {
                                 Id = a.v_OrganizationPadreId,
                                 Value1 = b.v_Name
                             }).Distinct().ToList();
        ...
        public static List<KeyValueDTO> GetAllEmpresa(ref OperationResult pobjOperationResult)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
        ...
        public static List<KeyValueDTO> GetAllEmpresaLocationPadre(ref OperationResult pobjOperationResult)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                ...
                var query = (from a in dbContext.organization
                             join b in dbContext.organization on a.v_OrganizationPadreId equals b.v_OrganizationId
                             join c in dbContext.location on b.v_OrganizationId equals c.v_OrganizationId
                             where a.i_IsDeleted == 0 && a.v_OrganizationPadreId != null
                             select new KeyValueDTO
                             {
                                 Id = a.v_OrganizationPadreId + "|" + c.v_LocationId,
                                 Value1 = b.v_Name
                             }).Distinct().ToList();
        ...
        public static List<KeyValueDTO> GetJoinOrganizationAndLocation_(ref OperationResult pobjOperationResult, int pintNodeId)
        {
            //Devart.Data.PostgreSql.PgSqlMonitor mon = new Devart.Data.PostgreSql.PgSqlMonitor();
            //mon.IsActive = true;
            ...
            try
            {
                ...
                var query = (from n in dbContext.node
                             ...
                             join b in dbContext.organization on J1.v_OrganizationId equals b.v_OrganizationId
                             join c in dbContext.location on a.v_LocationId equals c.v_LocationId
                             where //(n.i_NodeId == pintNodeId || n.i_NodeId == 3) &&
                                   n.i_IsDeleted == 0 &&
                                   a.i_IsDeleted == 0
                             ...
                             select new RestrictedWarehouseProfileList
                             {
                                 ...
                                 v_OrganizationId = b.v_OrganizationId,
                                 i_NodeId = J1.i_NodeId,
                                 v_OrganizationPadreId = b.v_OrganizationPadreId
                             }
                          );
        ...
        public static List<KeyValueDTO> GetJoinOrganizationPadreAndLocation(ref OperationResult pobjOperationResult, int pintNodeId)
        {
            //Devart.Data.PostgreSql.PgSqlMonitor mon = new Devart.Data.PostgreSql.PgSqlMonitor();
            //mon.IsActive = true;
            ...
            try
            {
                ...
                var query = (from n in dbContext.node
                             ...
                             join J1 in dbContext.nodeorganizationprofile on new { a = a.i_NodeId, b = a.v_OrganizationId }
                                                      equals new { a = J1.i_NodeId, b = J1.v_OrganizationId } into j1_join
                             from J1 in j1_join.DefaultIfEmpty()
                             join b in dbContext.organization on J1.v_OrganizationId equals b.v_OrganizationPadreId

                             join b1 in dbContext.organization on b.v_OrganizationPadreId equals b1.v_OrganizationId
                             join c in dbContext.location on a.v_LocationId equals c.v_LocationId
                             where n.i_NodeId == pintNodeId &&
                                   n.i_IsDeleted == 0 &&
                                   a.i_IsDeleted == 0
        ...
        public static List<KeyValueDTO> LlenarComboHijo(string pstrEmpresaPadreId)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
