...
namespace Sigesoft.Node.WinClient.BLL
{
    public class SystemParameterBL
    {
        ...
        public List<SystemParameterList> GetSystemParametersPagedAndFilteredNewHabitos(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, int? pintGroupId)
        {
            //mon.IsActive = true;
            ...
            try
            {
                ...
                var query = from A in dbContext.systemparameter       
                            ...
                            select new SystemParameterList
                            {
                                ...
                                i_ParentGroupId = A.i_GroupId,
                                i_ParentParameterId = A.i_ParentParameterId.Value,
                                v_NoxiousHabitsName = A.v_Value1,                                
                                v_Value2 = A.v_Value2,                            
                                i_IsDeleted = A.i_IsDeleted,
                                SI = false,
                                NO = true,
        ...
        public List<SystemParameterList> GetSystemParametersPagedAndFilteredNewFamiliares(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, int? pintGroupId)
        {
            //mon.IsActive = true;
            ...
            try
            {
                ...
                var query = from A in dbContext.systemparameter
                            join B in dbContext.systemparameter on new { a = A.i_ParentParameterId.Value, b = A.i_GroupId }
                                                                equals new { a = B.i_ParameterId, b = 149 } into B_join
                            from B in B_join.DefaultIfEmpty()
        ...
        public int ObtenerUltimoIdParamenter(int groupId)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                ...
                var objEntity = (from a in dbContext.systemparameter
                                 ...
                                 orderby a.i_ParameterId descending
                                 select a).FirstOrDefault();

                if (objEntity == null)
                    return 1;

                return objEntity.i_ParameterId + 1;
        ...
        public void ActualizarFlagUsuarioCovid19( int id, string flag)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                ...
                var objEntitySource = (from a in dbContext.systemparameter
                                       where a.i_GroupId == 407 && a.i_ParameterId == id
                                       select a).FirstOrDefault();

                objEntitySource.v_Value2 = flag;
        ...
        public bool ValidarUsuarioTemporal(string usuario, string pass) {

            SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();

            var usuarios = (from A in dbContext.systemparameter where A.i_GroupId == 407 select A).ToList();

            foreach (var item in usuarios)
            {
                var arr = item.v_Value1.Split('|');
                var username = arr[0];
                var password = arr[1];
                var tecnico = arr[2];
                var estado = bool.Parse(item.v_Value2);

                if (username == usuario && password == pass && estado)
                {
                    return true;
                }
            }
