...
namespace Sigesoft.Node.WinClient.UI.Operations.Popups
{
    public partial class frmAddMedicationMedicalConsult : Form
    {
        ...
        private void MedicamentSearch()
        {
            ...
            var medicaments = objLogistBL.GetProductWarehousePagedAndFiltered(ref objOperationResult, 0, null, filterExpression, _pharmacyWarehouseId);

            if (medicaments != null)
            {
                if (medicaments.Count > 0)
                {
                    grdMedicament.DataSource = medicaments;
                    grdMedicament.Rows[0].Selected = true;
                    lblRecordCount.Text = string.Format("Se encontraron {0} registros.", medicaments.Count());
                }
                else
                {
                    grdMedicament.DataSource = medicaments;
