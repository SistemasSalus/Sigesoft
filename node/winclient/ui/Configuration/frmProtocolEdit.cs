    ...
    public partial class frmProtocolEdit : Form
    {
        ...
        private void LoadData()
        {
            ...
            else if (_mode == "Edit")
            {
                ...
                cboStatusProtocolId.SelectedValue = _protocolDTO.v_StatusProtocolId.ToString();
                cboTypeReportProtocolId.SelectedValue = _protocolDTO.i_TypeReport.ToString();
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            if (uvProtocol.Validate(true, false).IsValid)
            {
                ...
                string[] componentIdFromProtocol = _tmpProtocolcomponentList.FindAll(p => p.i_RecordStatus != (int)RecordStatus.EliminadoLogico).OrderBy(o => o.v_ComponentId)
                ...
                {
                    DialogResult Result1 = MessageBox.Show("Está seguro de guardar el Protocolo como Aprobado por el Cliente", "Alerta", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                ...
                if (int.Parse(cbService.SelectedValue.ToString()) == 3)
                {
                    DialogResult Result1 = MessageBox.Show("Nuestro CSO sólo es Ocupacional no admite Consultas Médicas por Ahora!!!", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                //VALIDAR QUE FICHA MÉDICA SE SELECCIONA PARA EL PROTOCOLO
                if (int.Parse(cboTypeReportProtocolId.SelectedValue.ToString()) == 1)
                {
                    DialogResult Result1 = MessageBox.Show("Confirma elegir la Ficha 312, como ficha médica para el Protocolo?", "Atención", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (Result1 == DialogResult.No)
                    {
                        return;
                    }
                }
                else if (int.Parse(cboTypeReportProtocolId.SelectedValue.ToString()) == 2)
                {
                    DialogResult Result1 = MessageBox.Show("Confirma elegir el Anexo 16, como ficha médica para el Protocolo?", "Atención", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (Result1 == DialogResult.No)
                    {
                        return;
                    }
                }
                else if (int.Parse(cboTypeReportProtocolId.SelectedValue.ToString()) == 3)
                {
                    DialogResult Result1 = MessageBox.Show("Confirma elegir la Ficha 312 y Anexo 16, como fichas médicas para el Protocolo?!", "Atención", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    if (Result1 == DialogResult.No)
                    {
                        return;
                    }
                ...
                _protocolDTO.v_StatusProtocolId =int.Parse(cboStatusProtocolId.SelectedValue.ToString());
                _protocolDTO.i_TypeReport = int.Parse(cboTypeReportProtocolId.SelectedValue.ToString());
                ...
                if (_mode == "New" || _mode == "Clon")
                {
                   ...
                   gbDatosProtocolo.Enabled = false;
        ...
        private void LoadComboBox()
        {
            ...
            cbEsoType.SelectedValue = "2";
            Utils.LoadDropDownList(cboStatusProtocolId, "Value1", "Id", BLL.Utils.GetDataHierarchyForCombo(ref objOperationResult, 116, null), DropDownListAction.Select);
            Utils.LoadDropDownList(cboTypeReportProtocolId, "Value1", "Id", BLL.Utils.GetSystemParameterForCombo(ref objOperationResult, 272, null), DropDownListAction.Select);
            //cboTypeReportProtocolId.SelectedValue = "1";
            ...
            var dataListOrganization2 = BLL.Utils.GetAllEmpresaLocationPadre(ref objOperationResult1);
            ...
            //Utils.LoadDropDownList(cbOrganizationInvoice, "Value1", "Id", BLL.Utils.GetAllEmpresaPadre(ref objOperationResult), DropDownListAction.Select);
            ...
            if (_mode == "New")
            {
                cbServiceType.SelectedValue = "1";
                cbService.SelectedValue = "2";                
            }
            else
            {
                // combo servicio
        ...
        {
            var frm = new frmProtocolComponentEdit(string.Empty, string.Empty, "New");
        ...
        {
            var frm = new frmProtocolComponentEdit(_protocolComponentId, _componentId, "Edit");
        ...
        private void LoadcbGESO()
        {
            ...
            var dataList = cbOrganizationInvoice.SelectedValue.ToString().Split('|');
            string idOrg = dataList[0];
            string idLoc = dataList[1];

            OperationResult objOperationResult1 = new OperationResult();
            var x =  BLL.Utils.GetGESOByOrgIdAndLocationId(ref objOperationResult1, idOrg, idLoc);
            Utils.LoadDropDownList(cbGeso, "Value1", "Id",x, DropDownListAction.Select);
            cbGeso.SelectedValue = x[1].Id;
        }
            ...
            {      
                ...
                if (row != null)
                {
                    ...
                    if (row.Cells["v_ProtocolComponentId"].Value != null)
                    {
                        ...
                        _componentId = row.Cells["v_ComponentId"].Value.ToString();
                    }
        ...
        private void grdProtocolComponent_AfterSelectChange(object sender, Infragistics.Win.UltraWinGrid.AfterSelectChangeEventArgs e)
        {
            ...
            _componentId = grdProtocolComponent.Selected.Rows[0].Cells["v_ComponentId"].Value.ToString();
        }
        ...
        {
            var filterExpression = string.Format("v_Name.Contains(\"{0}\")","");
            var dataList = GetData1(0, null, "v_Name ASC", filterExpression);
            _tmpProtocolcomponentList = new List<ProtocolComponentList>();
            foreach (var item in dataList)
            {
                    ProtocolComponentList _protocolcomponent = new ProtocolComponentList();
                    _protocolcomponent.v_ProtocolComponentId = Guid.NewGuid().ToString();
                    _protocolcomponent.v_ComponentId = item.v_MedicalExamId;
                    _protocolcomponent.v_ComponentName = item.v_Name;
                    _protocolcomponent.r_Price = 1; //float.Parse(txtFinalPrice.Value.ToString());
                    _protocolcomponent.v_Operator = "";
                    _protocolcomponent.i_Age = 1;
                    _protocolcomponent.v_Gender = "Ambos";
                    _protocolcomponent.i_IsConditionalId = 0;
                    _protocolcomponent.v_IsConditional =  "No";
                    _protocolcomponent.i_OperatorId = -1;
                    _protocolcomponent.i_GenderId = -1;
            ...
            var dataList1 = _tmpProtocolcomponentList.FindAll(p => p.i_RecordStatus != (int)RecordStatus.EliminadoLogico);
            grdProtocolComponent.DataSource = new ProtocolComponentList();
            grdProtocolComponent.DataSource = dataList1;
            grdProtocolComponent.Refresh();
            lblRecordCount2.Text = string.Format("Se encontraron {0} registros.", dataList1.Count());
        }

        private List<MedicalExamList> GetData1(int pintPageIndex, int? pintPageSize, string pstrSortExpression, string pstrFilterExpression)
        {
        ...
        private void btnOFTbasico_Click(object sender, EventArgs e)
        {
            var filterExpression = string.Format("v_CategoryId.Contains(\"OFTALMO\")", "");
            var dataList = GetData1(0, null, "v_Name ASC", filterExpression);
            _tmpProtocolcomponentList = new List<ProtocolComponentList>();
            foreach (var item in dataList)
            {
