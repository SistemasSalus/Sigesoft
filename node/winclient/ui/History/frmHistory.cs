...
namespace Sigesoft.Node.WinClient.UI
{
    public partial class frmHistory : Form
    {
        ...
        List<SystemParameterList> _objSystemParameterListMedicoPersonal;
        List<SystemParameterList> _objSystemParameterListHabitos;
        List<SystemParameterList> _objSystemParameterListMedicoFamiliar;
        #endregion
        ...
        public frmHistory( string pstrPacientId)
        {
            ...
            #region Cargar Medico Personales
            //****************************************************************************************************//
            SystemParameterBL objSystemParameterBL = new SystemParameterBL();
            _objSystemParameterListMedicoPersonal = new List<SystemParameterList>();
            ...
            {
                _objSystemParameterListMedicoPersonal = objSystemParameterBL.GetSystemParametersPagedAndFilteredNew(ref objOperationResult, 0, null, "", "i_GroupId== 147 && i_IsDeleted == 0", 147);
            }
            else
            {
                foreach (var item in PersonMedicalHistoryList)
                {
                    ...
                    _objSystemParameterListMedicoPersonal.Add(objSystemParameter);
                }
                _objSystemParameterListMedicoPersonal = objSystemParameterBL.GetSystemParametersPagedAndFilteredNew(ref objOperationResult, 0, null, "", "i_GroupId== 147 && i_IsDeleted == 0", 147);
                foreach (var item in PersonMedicalHistoryList)
                {
                    SystemParameterList Result = _objSystemParameterListMedicoPersonal.Find(p => p.v_Value1 == item.v_DiseasesId);
            ...
            ultraGrid2.DataSource = _objSystemParameterListMedicoPersonal;
            ...
            #region Cargar Habitos Noscivos
             //****************************************************************************************************//

            _objSystemParameterListHabitos = new List<SystemParameterList>();
            SystemParameterList objSystemParameterHabitos = new SystemParameterList();

            List<NoxiousHabitsList> NoxiousHabitsList = new List<NoxiousHabitsList>();
            NoxiousHabitsList = oHistoryBL.GetNoxiousHabitsPagedAndFilteredByPersonId1(ref objOperationResult, 0, null, "", "", pstrPacientId);

            if (NoxiousHabitsList.Count == 0)
            {
                _objSystemParameterListHabitos = objSystemParameterBL.GetSystemParametersPagedAndFilteredNewHabitos(ref objOperationResult, 0, null, "", "i_GroupId== 148 && i_IsDeleted == 0", 148);
            }
            else
            {
                foreach (var item in NoxiousHabitsList)
                {
                    objSystemParameter.v_Value1 = item.v_NoxiousHabitsId;
                    objSystemParameter.v_DiseaseName = item.v_NoxiousHabitsName;
                    if (item.i_Answer == (int)SiNo.NO)
                    {
                        objSystemParameter.NO = true;
                    }
                    ...
                    _objSystemParameterListHabitos.Add(objSystemParameter);
                }
                _objSystemParameterListHabitos = objSystemParameterBL.GetSystemParametersPagedAndFilteredNewHabitos(ref objOperationResult, 0, null, "", "i_GroupId== 148 && i_IsDeleted == 0", 148);
                foreach (var item in NoxiousHabitsList)
                {
                    SystemParameterList Result = _objSystemParameterListHabitos.Find(p => p.i_ParameterId == item.i_TypeHabitsId);

                    if (Result == null)
                        Result = new SystemParameterList();

                    Result.i_Answer = item.i_Answer.Value;
            ...
            GridNoxiousHabits1.DataSource = _objSystemParameterListHabitos;
            #endregion

            #region Medicos Familiares

            _objSystemParameterListMedicoFamiliar = new List<SystemParameterList>();
          
       List<FamilyMedicalAntecedentsList> FamilyMedicalAntecedentsList = new List<FamilyMedicalAntecedentsList>();
       FamilyMedicalAntecedentsList = oHistoryBL.GetFamilyMedicalAntecedentsPagedAndFilteredByPersonId1(ref objOperationResult, 0, null, "", "", pstrPacientId);

            if (FamilyMedicalAntecedentsList.Count == 0)
            {
                _objSystemParameterListMedicoFamiliar = objSystemParameterBL.GetSystemParametersPagedAndFilteredNewFamiliares(ref objOperationResult, 0, null, "", "i_GroupId== 149 && i_IsDeleted == 0", 149);
            }
            else
            {
                foreach (var item in FamilyMedicalAntecedentsList)
                {
                    objSystemParameter.v_Value1 = item.v_DiseasesId;
                    objSystemParameter.v_DiseaseName = item.v_TypeFamilyName;
                    if (item.i_Answer == (int)SiNo.NO)
                    {
                        objSystemParameter.NO = true;
                    }
                    ...
                    _objSystemParameterListMedicoFamiliar.Add(objSystemParameter);
                }
                _objSystemParameterListMedicoFamiliar = objSystemParameterBL.GetSystemParametersPagedAndFilteredNewFamiliares(ref objOperationResult, 0, null, "", "i_GroupId== 149 && i_IsDeleted == 0", 149);
                foreach (var item in FamilyMedicalAntecedentsList)
                {
                    SystemParameterList Result = _objSystemParameterListMedicoFamiliar.Find(p => p.v_Value1 == item.v_DiseasesId && p.Enfermedad == item.v_TypeFamilyName);
            ...
            GridMedicoFamiliar.DataSource = _objSystemParameterListMedicoFamiliar;
            #endregion
        ...
        private void frmHistory_Load(object sender, EventArgs e)
        {
            ...
            this.GridMedicoFamiliar.Rows.ExpandAll(true);
        ...
        private void btnDeletePersonMedical_Click(object sender, EventArgs e)
        {
            if (_PersonMedicalHistoryId != "")
            {
               ...
               var x = _objSystemParameterListMedicoPersonal.Find(p => p.v_Value1 == findResult.v_DiseasesId);
               if (x != null)
               {
                   x.NO = true;
                   x.ND = false;
        ...
        private void btnSavePersonMedical_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in _objSystemParameterListMedicoPersonal)
            {
        ...
        private void btnSaveNoxiousHabits_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in _TempNoxiousHabitsList)
            {
                ...
                if (item.i_RecordType == (int)RecordType.Temporal && item.i_RecordStatus == (int)RecordStatus.Agregado)
                {
                    ...
                    noxioushabitsDto.i_AnswerId = item.i_Answer;
            ...
            foreach (var item in _objSystemParameterListHabitos)
            {
                if (item.i_Answer == (int)SiNo.NONE)
                {
                    noxioushabitsDto noxioushabitsDto = new noxioushabitsDto();

                    noxioushabitsDto.i_TypeHabitsId = item.i_ParameterId;
                    noxioushabitsDto.v_PersonId = _PacientId;
                    noxioushabitsDto.v_NoxiousHabitsId = item.v_Value1;
                    noxioushabitsDto.v_Frequency = String.Empty;
                    noxioushabitsDto.v_Comment = String.Empty;                
                    noxioushabitsDto.i_AnswerId = item.i_Answer;
        ...
        private void btnOtrosFamilyMedical_Click(object sender, EventArgs e)
        {
            ...
            if (findResult == null)
            {
                ...
                objFamilyMedicAntecedent.i_Answer = (int)SiNo.SI;
        ...
        private void btnSaveFamilyMedical_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in _TempFamilyMedicalAntecedentsList)
            {
                ...
                if (item.i_RecordType == (int)RecordType.Temporal && item.i_RecordStatus == (int)RecordStatus.Agregado)
                {
                    ...
                    familymedicalantecedentsDto.i_AnswerId = item.i_Answer;
            ...
            foreach (var item in _objSystemParameterListMedicoFamiliar)
            {
                if (item.i_Answer == (int)SiNo.NONE)
                {
                    familymedicalantecedentsDto familymedicalantecedentsDto = new familymedicalantecedentsDto();

                    familymedicalantecedentsDto.i_TypeFamilyId = item.i_ParameterId;
                    familymedicalantecedentsDto.v_PersonId = _PacientId;
                    familymedicalantecedentsDto.v_DiseasesId = item.v_Value1;
                    familymedicalantecedentsDto.v_Comment = String.Empty;
                    familymedicalantecedentsDto.i_AnswerId = item.i_Answer;
          ...
          //bool x =  ScrambledEquals(_TempPersonMedicalHistoryList, _TempPersonMedicalHistoryListOld);
            ...
            if (_ResultMedicoPersonales)
            {
                ...
                else
                {
                    ...
                    OperationResult objOperationResult = new OperationResult();
                    SystemParameterBL objSystemParameterBL = new SystemParameterBL();
                    var PersonMedicalHistoryList = new HistoryBL().GetPersonMedicalHistoryPagedAndFilteredByPersonId1(ref objOperationResult, 0, null, "", "", _PacientId);
                    ...
                    {
                        ultraGrid2.DataSource = PersonMedicalHistoryList;
                    }
        ...
        private void ultraGrid2_CellChange(object sender, CellEventArgs e)
        {
            ...
            if (e.Cell.Column.Key == "SI" && e.Cell.Text == "True")
            {
                ...
                var rr = _objSystemParameterListMedicoPersonal.Find(p => p.i_ParameterId == (int)i_ParameterId);
            ...
            if (e.Cell.Column.Key == "NO" && e.Cell.Text == "True")
            {
                ...
                else
                {
                    ...
                    var found = new ServiceBL().GetPersonMedicalHistoryForESOBuildMenu(_PacientId, rr.v_Value1);

                    if (found != null)
                    {
                        //el antecedente existe como ND 
                        if (found.i_AnswerId == (int)SiNo.NONE)  // No define
                        {
                            // este item no esta en la grilla
                            // caso que viene de ND y se cambie a NO
                            var newitem = new PersonMedicalHistoryList();
                            newitem.i_RecordType = (int)RecordType.NoTemporal;
                            newitem.v_PersonMedicalHistoryId = found.v_PersonMedicalHistoryId;
                            newitem.v_DiseasesName = rr.v_DiseasesName;
                            newitem.i_RecordStatus = (int)RecordStatus.Modificado;
                            newitem.v_PersonId = _PacientId;
                            newitem.i_Answer = (int)SiNo.NO;
                            newitem.v_DiseasesId = rr.v_Value1;
                            _TempPersonMedicalHistoryList.Add(newitem);
                        }
                    }
            ...
            if (e.Cell.Column.Key == "SI")
            {
                ...
                if (frm.DialogResult != System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    var x = _objSystemParameterListMedicoPersonal.Find(p => p.v_Value1 == value1);
        ...
        private void GridNoxiousHabits1_InitializeLayout(object sender, InitializeLayoutEventArgs e)
        {
            UltraGridLayout layout = e.Layout;
            UltraGridBand band = layout.Bands[0];
            ...
            foreach (UltraGridColumn oCol in oBand.Columns)
            {
                if (oCol.Key == "v_NoxiousHabitsName")
                {
                    oCol.CellActivation = Activation.NoEdit;
                }
            }
            ...
            if (band.SortedColumns.Count > 0)
            {
                band.SortedColumns.Add("v_NoxiousHabitsName", false, true);
            }
        }
      
        private void GridNoxiousHabits1_CellChange(object sender, CellEventArgs e)
        {
            OperationResult objOperationResult = new OperationResult();
            SystemParameterBL objSystemParameterBL = new SystemParameterBL();
            SystemParameterList objSystemParameterList = new SystemParameterList();
            NoxiousHabitsList objNoxiousHabits = new NoxiousHabitsList();
            int OldAnswer = 0;
            _ResultlHabitosNoxivos = true;
            ...
            {
                var i_ParameterId = e.Cell.Row.Cells["i_ParameterId"].Value;
                var rr = _objSystemParameterListHabitos.Find(p => p.i_ParameterId == (int)i_ParameterId);
                OldAnswer = rr.i_Answer;
                rr.i_Answer = (int)SiNo.SI;
            ...
            if (e.Cell.Column.Key == "NO" && e.Cell.Text == "True")
            {
                ...
                // si es no, buscar en la bolsa si esta, si se encuentra cambiar de flag en la _TempNoxiousHabitsList a un eliminado lógico
                var findResult = _TempNoxiousHabitsList.Find(p => p.i_TypeHabitsId == rr.i_ParameterId);
                if (findResult != null)
                {
                    findResult.i_RecordStatus = (int)RecordStatus.EliminadoLogico;
                    findResult.i_Answer = (int)SiNo.NO;
                ...
                {
                    //Verificar si existe el habito noscivo
                    var found = new ServiceBL().GetHabitosHistoryForESOBuildMenu(_PacientId, rr.i_ParameterId);
                    ...
                    if (found != null)
                    {
                        ...
                        if (found.i_AnswerId == (int)SiNo.NONE)  // No define
                        {
                            // este item no esta en la grilla
                            // caso que viene de ND y se cambie a NO
                            var newitem = new NoxiousHabitsList();
                            newitem.i_RecordType = (int)RecordType.NoTemporal;
                            newitem.v_NoxiousHabitsId = found.v_NoxiousHabitsId;
                            newitem.v_NoxiousHabitsName = rr.v_NoxiousHabitsName;
                            newitem.i_RecordStatus = (int)RecordStatus.Modificado;
                            newitem.v_PersonId = _PacientId;
                            newitem.i_Answer = (int)SiNo.NO;
                            newitem.i_TypeHabitsId = rr.i_ParameterId;
                            _TempNoxiousHabitsList.Add(newitem);
                        }
                    }
            ...
            if (e.Cell.Column.Key == "SI")
            {
                ...
                string NoxiousHabitsName = e.Cell.Row.Cells["v_NoxiousHabitsName"].Value.ToString();
                int ParameterId = int.Parse(e.Cell.Row.Cells["i_ParameterId"].Value.ToString());
                //string Value2 = e.Cell.Row.Cells["v_Value2"].Value.ToString();
                //objSystemParameterList = objSystemParameterBL.GetParentNameSystemParameter(ref objOperationResult, value1, 148);

                //if (objSystemParameterList == null) return;

                History.frmNoxiousHabitsPopup frm = new History.frmNoxiousHabitsPopup(NoxiousHabitsName, "", "");
                frm.ShowDialog();
                if (frm.DialogResult != System.Windows.Forms.DialogResult.OK)
                {

                    var x = _objSystemParameterListHabitos.Find(p => p.i_ParameterId == ParameterId);
                    x.i_Answer = OldAnswer;
                    if (OldAnswer == (int)SiNo.NO)
                    {
                        x.NO = true;
                    ...
                    GridNoxiousHabits1.DataSource = new List<SystemParameterList>();
                    GridNoxiousHabits1.Refresh();
                    GridNoxiousHabits1.DataSource = _objSystemParameterListHabitos;
                    this.grdDataNoxiousHabits.Rows.ExpandAll(true);
                    return;
                }
                //string DiseasesId = value1;

                //Busco en la lista temporal si ya se agrego el item seleccionado
                var findResult = _TempNoxiousHabitsList.Find(p => p.v_NoxiousHabitsName == NoxiousHabitsName);
                if (findResult == null)
                {
                    objNoxiousHabits.v_NoxiousHabitsId = Guid.NewGuid().ToString();
                    objNoxiousHabits.v_PersonId = _PacientId;
                    objNoxiousHabits.v_Frequency = frm._Frequency;
                    objNoxiousHabits.v_Comment = frm._Comment;
                    objNoxiousHabits.i_TypeHabitsId = ParameterId;
                    objNoxiousHabits.v_TypeHabitsName = NoxiousHabitsName;
                    objNoxiousHabits.i_RecordStatus = (int)RecordStatus.Agregado;
                    objNoxiousHabits.i_RecordType = (int)RecordType.Temporal;
                    objNoxiousHabits.i_Answer = 1; // SI
                    _TempNoxiousHabitsList.Add(objNoxiousHabits);
                }
                else
                {
                    if (findResult.i_RecordStatus == (int)RecordStatus.EliminadoLogico)
        ...
        #region Medicos Familiares
        private void GridMedicoFamiliar_InitializeLayout(object sender, InitializeLayoutEventArgs e)
        {
            UltraGridLayout layout = e.Layout;
            UltraGridBand band = layout.Bands[0];
        ...
        private void GridMedicoFamiliar_CellChange(object sender, CellEventArgs e)
        {

            OperationResult objOperationResult = new OperationResult();
            SystemParameterBL objSystemParameterBL = new SystemParameterBL();
            SystemParameterList objSystemParameterList = new SystemParameterList();
            FamilyMedicalAntecedentsList objFamilyMedicalAntecedent = new FamilyMedicalAntecedentsList();
            int OldAnswer = 0;
            _ResultMedicoFamiliares = true;
            ...
            {
                var i_ParameterId = e.Cell.Row.Cells["i_ParameterId"].Value;
                var rr = _objSystemParameterListMedicoFamiliar.Find(p => p.i_ParameterId == (int)i_ParameterId);
                OldAnswer = rr.i_Answer;
                rr.i_Answer = (int)SiNo.SI;
            ...
            if (e.Cell.Column.Key == "NO" && e.Cell.Text == "True")
            {
                ...
                // si es no, buscar en la bolsa si esta, si se encuentra cambiar de flag en la _TempFamilyMedicalAntecedentsList a un eliminado lógico
                var findResult = _TempFamilyMedicalAntecedentsList.Find(p => p.v_DiseasesId == rr.v_Value1);
                if (findResult != null)
                {
                    findResult.i_RecordStatus = (int)RecordStatus.EliminadoLogico;
                    findResult.i_Answer = (int)SiNo.NO;
                ...
                {
                    //Verificar si existe el antecedente medico personal
                    var found = new ServiceBL().GetPersonaFamiliarForESOBuildMenu(_PacientId, rr.v_Value1);
                    ...
                    if (found != null)
                    {
                        ...
                        if (found.i_AnswerId == (int)SiNo.NONE)  // No define
                        {
                            // este item no esta en la grilla
                            // caso que viene de ND y se cambie a NO
                            var newitem = new FamilyMedicalAntecedentsList();
                            newitem.i_RecordType = (int)RecordType.NoTemporal;
                            newitem.v_FamilyMedicalAntecedentsId = found.v_FamilyMedicalAntecedentsId;
                            newitem.v_DiseaseName = rr.v_DiseasesName;
                            newitem.i_RecordStatus = (int)RecordStatus.Modificado;
                            newitem.v_PersonId = _PacientId;
                            newitem.i_Answer = (int)SiNo.NO;
                            newitem.v_DiseasesId = rr.v_Value1;
                            _TempFamilyMedicalAntecedentsList.Add(newitem);
                        }
                    }
            ...
            if (e.Cell.Column.Key == "SI")
            {
                ...
                string v_DiseasesId = e.Cell.Row.Cells["v_Value1"].Value.ToString();
                string v_DiseaseName = e.Cell.Row.Cells["v_DiseaseName"].Value.ToString();
                string Grupo = e.Cell.Row.Cells["Enfermedad"].Value.ToString(); 
                
                int ParameterId = int.Parse(e.Cell.Row.Cells["i_ParameterId"].Value.ToString());
                objSystemParameterList = objSystemParameterBL.GetParentNameSystemParameter(ref objOperationResult, v_DiseasesId, 149);

                if (objSystemParameterList == null) return;

                History.frmFamilyMedicPopup frm = new History.frmFamilyMedicPopup(v_DiseaseName, "", Grupo);
                frm.ShowDialog();
                if (frm.DialogResult != System.Windows.Forms.DialogResult.OK)
                {

                    var x = _objSystemParameterListMedicoFamiliar.Find(p => p.v_Value1 == v_DiseasesId);
                    x.i_Answer = OldAnswer;
                    if (OldAnswer == (int)SiNo.NO)
                    {
                        x.NO = true;
                    ...
                    ultraGrid2.DataSource = new List<SystemParameterList>();
                    ultraGrid2.Refresh();
                    ultraGrid2.DataSource = _objSystemParameterListMedicoFamiliar;
                    this.ultraGrid2.Rows.ExpandAll(true);
                    return;
                }
                string DiseasesId = v_DiseasesId;

                //Busco en la lista temporal si ya se agrego el item seleccionado
                var findResult = _TempFamilyMedicalAntecedentsList.Find(p => p.v_DiseasesId == DiseasesId && p.i_TypeFamilyId == ParameterId); // && p.d_StartDate == frm._StartDate
                if (findResult == null)
                {
                    objFamilyMedicalAntecedent.v_FamilyMedicalAntecedentsId = Guid.NewGuid().ToString();
                    objFamilyMedicalAntecedent.v_PersonId = _PacientId;
                    objFamilyMedicalAntecedent.v_DiseasesId = DiseasesId;
                    objFamilyMedicalAntecedent.v_TypeFamilyName = Grupo;
                    objFamilyMedicalAntecedent.i_TypeFamilyId = ParameterId;
                    objFamilyMedicalAntecedent.v_DiseaseName = v_DiseaseName;
                    objFamilyMedicalAntecedent.v_Comment = frm._CommentFamilyMedic;
                    objFamilyMedicalAntecedent.i_RecordStatus = (int)RecordStatus.Agregado;
                    objFamilyMedicalAntecedent.i_RecordType = (int)RecordType.Temporal;
                    objFamilyMedicalAntecedent.i_Answer = 1; // SI
                    _TempFamilyMedicalAntecedentsList.Add(objFamilyMedicalAntecedent);
                }
                else
                {
                    if (findResult.i_RecordStatus == (int)RecordStatus.EliminadoLogico)
