...
{
    public partial class frmConsultationService : Form
    {
        ...
        private void BindGrid()
        {
            ...
            if (chkAgruparPorPaciente.Checked)   // agrupado
            {
                grdDataGrouping.DataSource = objData;
            }
            else    // detallado
            {
                grdDataDetail.DataSource = objData;
            }

            btnExportExcel.Enabled = btnExportPdf.Enabled = ((grdDataGrouping.Rows.Count > 0 && chkAgruparPorPaciente.Checked) || (grdDataDetail.Rows.Count > 0 && chkAgruparPorPaciente.Checked == false));
        ...
        private List<ServiceList> GetData(int pintPageIndex, int? pintPageSize, string pstrSortExpression, string pstrFilterExpression)
        {
            ...
            List<ServiceList> objData = null;

            if (chkAgruparPorPaciente.Checked)   // agrupado
            {
                objData = _objServiceBL.GetServicesPagedAndFilteredReport(ref objOperationResult, pintPageIndex, pintPageSize, pstrSortExpression, pstrFilterExpression, pdatBeginDate, pdatEndDate);
                grdDataDetail.Visible = false;
                grdDataGrouping.Visible = true;
            } 
            else    // detallado
            {
                objData = _objServiceBL.GetServicesDetailPagedAndFilteredReport(ref objOperationResult, pintPageIndex, pintPageSize, pstrSortExpression, pstrFilterExpression, pdatBeginDate, pdatEndDate);
                grdDataGrouping.Visible = false;
                grdDataDetail.Visible = true;
        ...
        private void frmConsultationService_Load(object sender, EventArgs e)
        {
            OperationResult objOperationResult = new OperationResult();
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            {
                if (grdDataGrouping.Visible)   // agrupado
                {
                    this.ultraGridExcelExporter1.Export(this.grdDataGrouping, saveFileDialog1.FileName);
                }
                else    // detallado
                {
                    this.ultraGridExcelExporter1.Export(this.grdDataDetail, saveFileDialog1.FileName);
                }     
        ...
        private void btnExportPdf_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                if (grdDataGrouping.Visible)   // agrupado
                {
                    this.ultraGridDocumentExporter1.Export(this.grdDataGrouping, saveFileDialog1.FileName, GridExportFileFormat.PDF);
                }
                else    // detallado
                {
                    this.ultraGridDocumentExporter1.Export(this.grdDataDetail, saveFileDialog1.FileName, GridExportFileFormat.PDF);
                }    
