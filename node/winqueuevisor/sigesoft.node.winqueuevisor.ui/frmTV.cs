...
namespace Sigesoft.Node.WinQueueVisor.UI
{
    public partial class frmTV : Form
    {
        ...
        private void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            {
                if (item.i_QueueStatusId == (int)QueueStatusId.LLAMANDO && item.i_Iscalling == (int)SiNo.SI)
                {
                    ...
                    for (int i = 0; i < int.Parse(Repetitions.ToString()); i++)
                    {
                        ...
                        //synth.SpeakAsync(string.Format(CallFormat, item.v_Pacient, item.v_CategoryName, item.v_OfficeNumber, VTrabajador));
                        synth.SpeakAsync(string.Format(CallFormat, item.v_Pacient, item.v_CategoryName, item.InformacionAdicional));                       
                    }    
                }
            }
            ...
            foreach (var item in x)
            {
                ...
                objServiceBL.UpdateServiceComponentVisor(ref objOperationResult, ServiceComponentId, (int)SiNo.NO);
            }
