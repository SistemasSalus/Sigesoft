...
namespace Sigesoft.Common
{
    public class Utils
    {
        ...
        public static void SendMessage(string emailHost, int port, bool enableSsl, bool EsHTML, string Desde, string DesdePassword, string Para, string CopiarA, string CopiarBcc, string Asunto, string Mensaje, List<string> AttachedFiles)
        {
            // Obtener los correos de los destinatarios en un array
            string[] ArrayPara = Para.Split(';');
            string[] ArrayCopiaA = CopiarA.Split(';');
            string[] ArrayCopiaBcc = CopiarBcc.Split(';');
            ...
            foreach (string strCopiarBcc in ArrayCopiaBcc)
                if (strCopiarBcc != "") oMail.Bcc.Add(strCopiarBcc.Trim());
        ...
        #region Decription
        public static string Decrypt(string pData)
        {
            UnicodeEncoding parser = new UnicodeEncoding();
            byte[] _original = parser.GetBytes(pData);
            MD5CryptoServiceProvider Hash = new MD5CryptoServiceProvider();
        ...
        public static void ClearContentControls(Control container)
        {
            foreach (Control control in container.Controls)
            {
                if (control.Controls.Count > 0)
                {
                    ClearContentControls(control);
                }
                else
                {
                    ...
                    if (control is Label) ((Label)control).Text = string.Empty;
                }
            }
        }

        public static void ClearContentControlsByCondition(Control container)
        {
            foreach (Control control in container.Controls)
            {
                if (control.Controls.Count > 0)
                {
                    ClearContentControlsByCondition(control);
                }
                else
                {
                    if (control is TextBox) ((TextBox)control).Clear();
                    ...
                    {
                        ((ComboBox)control).SelectedValue = "-1";
                    }
                }
            }
        }

        public static void ClearContentControlsByGroupOccupational(Control container)
        {
            foreach (Control control in container.Controls)
            {
                if (control.Controls.Count > 0)
        ...
        public static void OnlyValidateNumbers(KeyPressEventArgs e)
        {
